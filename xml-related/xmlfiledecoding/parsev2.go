...
func main() {
	...
	type EpisodeInfo struct {
		...
		Id            int      `xml:"id"`
		EpisodeNumber float32  `xml:"EpisodeNumber"`
	...
	type Result struct {
		Series      SeriesInfo
		EpisodeList []EpisodeInfo `xml:"Episode"`
	}
...
`
	...
	fmt.Println(v.Series.XMLName)
	fmt.Println(v.Series.SeriesName)
	fmt.Println(v.Series.Overview)
	fmt.Println(v.Series.Actors)
	fmt.Println(v.Series.Id)
	fmt.Println(v.Series.Rating)

	for _, e := range v.EpisodeList {
		fmt.Println(e.XMLName)
		fmt.Println(e.Id)
		fmt.Println(e.EpisodeNumber)
		fmt.Println(e.EpisodeName)
		fmt.Println(e.Overview)
		fmt.Println(e.Rating)
	}
